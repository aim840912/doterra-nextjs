...
class DeepBlueScraper {
  ...
  private async loadExistingProducts(): Promise<void> {
    const productFiles = [
      ...
      'src/data/products/deep-blue-collection.json' // 新增：載入現有 Deep Blue 產品以避免重複
    ];
  ...
  private determineCategory(productName: string): string {
    ...
    // 護膚產品
    if (name.includes('軟膏') || name.includes('乳液') || name.includes('護膚') || 
        name.includes('面霜') || name.includes('膏') || name.includes('cream') ||
        name.includes('lotion') || name.includes('rub')) {
      return 'skincare';
    }
  ...
  async getProductLinks(): Promise<ProductInfo[]> {
    ...
    const baseUrl = 'https://www.doterra.com/TW/zh_TW/pl/deep-blue-products';
    ...
    console.log(`✅ 找到 ${products.length} 個 Deep Blue 產品`);
  ...
  async scrapeProduct(productInfo: ProductInfo): Promise<Oil | null> {
    ...
    try {
      ...
      const oil: Oil = {
        ...
        collections: ['deep-blue'], // 標記為 Deep Blue 系列
  ...
  private getTargetFile(category: string): string {
    switch (category) {
      ...
      default:
        // 為 Deep Blue 系列產品創建專門檔案
        const deepBlueFile = 'src/data/products/deep-blue-collection.json';
        if (!fs.existsSync(deepBlueFile)) {
          fs.writeFileSync(deepBlueFile, '[]', 'utf-8');
        }
        return deepBlueFile;
    }
  }
  ...
  async run(): Promise<void> {
    try {
      ...
      console.log(`\n🚀 開始爬取 ${products.length} 個 Deep Blue 產品\n`);
...
// 執行爬蟲
async function main() {
  const scraper = new DeepBlueScraper();
  await scraper.run();
}
...
export default DeepBlueScraper;