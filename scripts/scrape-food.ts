...
class FoodScraper {
  ...
  private async loadExistingProducts(): Promise<void> {
    const productFiles = [
      ...
      'src/data/products/onguard-collection.json',
      'src/data/products/deep-blue-collection.json',
      'src/data/products/breathe-collection.json',
      'src/data/products/food-collection.json' // 新增：載入現有 Food 產品以避免重複
    ];
  ...
  private determineCategory(productName: string): string {
    ...
    // 護膚產品
    if (name.includes('膏') || name.includes('乳液') || name.includes('護膚') || 
        name.includes('面霜') || name.includes('cream') || name.includes('lotion') ||
        name.includes('rub') || name.includes('balm') || name.includes('牙膏') ||
        name.includes('漱口')) {
      return 'skincare';
    }
    
    // 健康產品/營養補充品（膠囊類歸類到 wellness）
    if (name.includes('膠囊') || name.includes('supplements') || 
        name.includes('營養') || name.includes('保健') || name.includes('softgels') ||
        name.includes('軟膠囊') || name.includes('supplement')) {
      return 'wellness';
    }
    
    // 其他 Food 產品默認分類（食品、調味品、飲品等）
    return 'food-collection';
  }
  ...
  async getProductLinks(): Promise<ProductInfo[]> {
    ...
    const baseUrl = 'https://www.doterra.com/TW/zh_TW/pl/food-products';
    ...
    console.log(`✅ 找到 ${products.length} 個 Food 產品`);
    ...
    try {
      ...
      const productData = await page.evaluate(() => {
        ...
        const volumeMatch = pageText.match(/(\d+)\s*毫升/) || pageText.match(/(\d+)\s*ml/i) ||
                           pageText.match(/(\d+)\s*g/i) || pageText.match(/(\d+)\s*公克/);
        if (volumeMatch) {
          const unit = pageText.includes('毫升') || pageText.includes('ml') ? 'ml' : 
                      pageText.includes('公克') || pageText.includes('g') ? 'g' : '';
          result.volume = volumeMatch[1] + unit;
        }
      ...
      for (const [title, content] of Object.entries(h2Contents)) {
        ...
        if (titleLower.includes('主要功效') || titleLower.includes('功效') || titleLower.includes('benefits')) {
        ...
        } else if (titleLower.includes('成分') || titleLower.includes('組成') || titleLower.includes('營養標示')) {
          productData.mainIngredients = this.splitMainBenefits(contentStr);
        } else if (titleLower.includes('使用方法') || titleLower.includes('使用') || titleLower.includes('食用方法')) {
      ...
      const oil: Oil = {
        ...
        collections: ['food'], // 標記為 Food 系列
        volume: productData.volume || '',
        imageUrl: productData.imageUrl || '',
        productIntroduction: productData.productIntroduction || '',
        applicationGuide: productData.applicationGuide || '', // 應用指南
  ...
  private getTargetFile(category: string): string {
    switch (category) {
      ...
      default:
        // 為 Food 系列產品創建專門檔案
        const foodFile = 'src/data/products/food-collection.json';
        if (!fs.existsSync(foodFile)) {
          fs.writeFileSync(foodFile, '[]', 'utf-8');
        }
        return foodFile;
    }
  }
  ...
  async run(): Promise<void> {
    try {
      ...
      console.log(`\n🚀 開始爬取 ${products.length} 個 Food 產品\n`);
