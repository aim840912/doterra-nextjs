...
class BreatheScraper {
  ...
  private async loadExistingProducts(): Promise<void> {
    const productFiles = [
      ...
      'src/data/products/breathe-collection.json' // 新增：載入現有 Breathe 產品以避免重複
    ];
  ...
  private determineCategory(productName: string): string {
    ...
    if (name.includes('膏') || name.includes('乳液') || name.includes('護膚') || 
        ...
        name.includes('rub') || name.includes('balm')) {
      return 'skincare';
    }
  ...
  async getProductLinks(): Promise<ProductInfo[]> {
    ...
    const baseUrl = 'https://www.doterra.com/TW/zh_TW/pl/breathe-products';
  ...
  async scrapeProduct(productInfo: ProductInfo): Promise<Oil | null> {
    ...
    try {
      ...
      const oil: Oil = {
        ...
        collections: ['breathe'], // 標記為 Breathe 系列
  ...
  private getTargetFile(category: string): string {
    switch (category) {
      ...
      default:
        // 為 Breathe 系列產品創建專門檔案
        const breatheFile = 'src/data/products/breathe-collection.json';
        if (!fs.existsSync(breatheFile)) {
          fs.writeFileSync(breatheFile, '[]', 'utf-8');
        }
        return breatheFile;
    }
  }
  ...
  async run(): Promise<void> {
    try {
      ...
      console.log(`\n🚀 開始爬取 ${products.length} 個 Breathe 產品\n`);
...
// 執行爬蟲
async function main() {
  const scraper = new BreatheScraper();
  await scraper.run();
}
